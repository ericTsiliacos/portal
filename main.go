...
func main() {
	...
	commando.
		...
		SetAction(func(args map[string]commando.ArgValue, flags map[string]commando.FlagValue) {
			...
			if !checkForDirtyIndex() && !checkForUnpublishedWork() {
			...
			sha := getRefBoundary(remoteTrackingBranch, currentBranch)
			...
			writePortalMetaData(currentBranch, sha)
	...
	commando.
		...
		SetAction(func(args map[string]commando.ArgValue, flags map[string]commando.FlagValue) {
			...
			_, _ = execute(fmt.Sprintf("git checkout %s", portalBranch))
			_, _ = execute(fmt.Sprintf("git pull -r origin %s", portalBranch))
			...
			commands := []string{
				fmt.Sprintf("git checkout %s", workingBranch),
				fmt.Sprintf("git pull -r origin %s", workingBranch),
				fmt.Sprintf("git reset --hard %s", sha),
				fmt.Sprintf("git rebase %s", portalBranch),
				"git reset HEAD^",
				fmt.Sprintf("git branch -D %s", portalBranch),
...
func getRefBoundary(remoteTrackingBranch string, currentBranch string) string {
	...
	if len(revisionBoundaries) > 0 {
		var boundaries = strings.Split(revisionBoundaries, "\n")
...
func writePortalMetaData(branch string, sha string) {
...
func checkForUnpublishedWork() bool {
